...
class SchemaLoader:
    ...
    def resolve(self, schema_def: SchemaDefinition) -> tuple[ResolvedSchema, ResolvedSchema]:
        ...
        input_rs = ResolvedSchema(
            json_schema=resolved_input, model=input_model, module_id=schema_def.module_id, direction="input"
        )
        output_rs = ResolvedSchema(
            json_schema=resolved_output, model=output_model, module_id=schema_def.module_id, direction="output"
        )
    ...
    def _schema_to_field_info(self, prop_schema: dict[str, Any], prop_name: str, parent_name: str) -> tuple[Any, Any]:
        ...
        if "oneOf" in prop_schema:
            types = [self._schema_to_type(s, f"{prop_name}_oneOf_{i}", parent_name) for i, s in enumerate(prop_schema["oneOf"])]
            return Union[tuple(types)], Field(default=...)  # type: ignore[valid-type]

        if "anyOf" in prop_schema:
            types = [self._schema_to_type(s, f"{prop_name}_anyOf_{i}", parent_name) for i, s in enumerate(prop_schema["anyOf"])]
            return Union[tuple(types)], Field(default=...)  # type: ignore[valid-type]
    ...
    def _handle_all_of(self, sub_schemas: list[dict[str, Any]], prop_name: str, parent_name: str) -> Any:
        ...
        merged_schema = {"type": "object", "properties": merged_properties, "required": list(set(merged_required))}
    ...
    def _wrap_native(
        self, module_id: str, input_model: type[BaseModel], output_model: type[BaseModel]
    ) -> tuple[ResolvedSchema, ResolvedSchema]:
