...
class Registry:
    ...
    def __init__(
        ...
        if extensions_dir is not None:
            ...
            self._extension_roots = [
                {"root": item} if isinstance(item, str) else item for item in extensions_dirs
            ]
        ...
        self._callbacks: dict[str, list[Callable[..., Any]]] = {"register": [], "unregister": []}
    ...
    def discover(self) -> int:
        ...
        if registered_count == 0 and discovered:
            logger.warning("No modules successfully registered from %d discovered files", len(discovered))
    ...
    def list(self, tags: list[str] | None = None, prefix: str | None = None) -> list[str]:
        ...
        if tags is not None:
            ...
            def has_all_tags(mid: str) -> bool:
                ...
                meta_tags = self._module_meta.get(mid, {}).get("tags", [])
                if meta_tags:
    ...
    def _trigger_event(self, event: str, module_id: str, module: Any) -> None:
        ...
        for cb in self._callbacks.get(event, []):
            try:
                ...
                logger.error("Callback error for event '%s' on module '%s': %s", event, module_id, e)
